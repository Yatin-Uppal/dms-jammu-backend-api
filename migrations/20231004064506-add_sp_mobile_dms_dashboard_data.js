...
BEGIN
    ...
    SET var_where_condition = CONCAT(
        'deleted_at IS NULL
         AND created_at >= @start_date_param
         AND created_at <  @end_date_param
         AND TRIM(series) = TRIM(''', series_param, ''')'
    );
    ...
    LEFT JOIN (
        SELECT
            ...
            MAX(
                CASE
                    WHEN TRIM(series) = TRIM(''', series_param, ''') THEN
                        CASE
                            WHEN (
                                SELECT IFNULL(d1.end, ''2'')
                                FROM driver_vehicle_details d1
                                WHERE d1.record_id = a.record_id
                                  AND d1.deleted_at IS NULL
                            ) != ''2'' THEN ''Green''

                            WHEN (
                                SELECT IFNULL(d2.begin, ''1'')
                                FROM driver_vehicle_details d2
                                WHERE d2.record_id = a.record_id
                            ...
                            WHEN (
                                ...
                                WHERE d3.record_id = a.record_id
    ...
    WHERE b.deleted_at IS NULL
      AND b.created_at >= ''', start_date_param, '''
      AND b.created_at <  ''', DATE_ADD(start_date_param, INTERVAL 52 HOUR), '''
      AND TRIM(b.series) = TRIM(''', series_param, ''')
    ORDER BY b.id
    ');
